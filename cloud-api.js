(function(){
  ...
  async function upsertProducts(products){const rows=products.map(toRow);try{return await request('/rest/v1/products?on_conflict=id',{method:'POST',admin:true,body:rows,prefer:'resolution=merge-duplicates,return=minimal'})}catch(error){if(!/season|column/i.test(error.message||''))throw error;return request('/rest/v1/products?on_conflict=id',{method:'POST',admin:true,body:rows.map(({season,...row})=>row),prefer:'resolution=merge-duplicates,return=minimal'})}}
